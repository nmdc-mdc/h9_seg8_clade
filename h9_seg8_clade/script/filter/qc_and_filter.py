...
def copy_files_recursively(source_pattern, destination_dir):
    ...
    for file_path in files:
        ...
        # If it's a directory, copy the entire directory recursively
            ...
            destination_subdir = os.path.join(destination_dir, os.path.basename(file_path))
...
def get_input_seq(parse_output):
    ...
    input_seqs = {}
    for f in files:
        f_path = os.path.join(parse_output, f)
...
if __name__ == "__main__":
    ...
    # 读取元数据表
    meta_df = pd.read_csv(os.path.join(work_dir, "/h9_seg8_clade/data/all_meta_data_with_vari.csv"), dtype="str",
                          keep_default_na=False)
    # 读取参考序列
    with open(os.path.join(work_dir, "/h9_seg8_clade/data/reference_seq.fasta")) as file:
        ref_seq = {x.split("\n")[0]: "".join(x.split("\n")[1:])
    ...
    # 生成比对变异的文件
    isolate_name_path = os.path.join(work_dir,f"/h9_seg8_clade/script/filter/filter_result/{isolate_name}")
    if os.path.exists(isolate_name_path):
        shutil.rmtree(isolate_name_path)
    os.makedirs(isolate_name_path)
    query_path = os.path.join(work_dir,f"/h9_seg8_clade/script/filter/filter_result/{isolate_name}/querys")
    os.makedirs(query_path)
    for seq_name, seq_info in seq_qc_filter_pass.items():
        seq_seq = seq_info["seq_seq"]
        seq_ref = seq_info["seq_ref_name"]
        seq_file_name = os.path.join(query_path,
                                     seq_ref + "___querys.fasta")
        with open(seq_file_name, "a") as f:
            f.write(">" + seq_name + "\n" + "\n".join(seq_seq) + "\n")
    annotation_isolate = os.path.join(work_dir,f"/h9_seg8_clade/script/call_snp_and_annotation/work_dir/{isolate_name}")
    if os.path.exists(annotation_isolate):
        shutil.rmtree(annotation_isolate)
    os.makedirs(annotation_isolate)
    annotation_isolate_input = os.path.join(work_dir,f"/h9_seg8_clade/script/call_snp_and_annotation/work_dir/{isolate_name}/input")
    os.makedirs(annotation_isolate_input)
    annotation_isolate_input_query = os.path.join(work_dir,
                                            f"/h9_seg8_clade/script/call_snp_and_annotation/work_dir/{isolate_name}/input/querys")
