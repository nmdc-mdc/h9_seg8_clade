...
def input_seq_add_to_tree(tree_path, output_df, output_dir):
    seg_tree = {}
    output_dir = os.path.join(output_dir, "tree")
    ...
    for seg, sub_df in groups:
        ...
        output_path = os.path.join(output_dir, f"{seg}.nwk")
...
if __name__ == '__main__':
    ...
    for f in os.listdir(f"{work_dir}/h9_seg8_clade/data/feature_json"):
        ...
        f_path = os.path.join(f"{work_dir}/h9_seg8_clade/data/feature_json", f)
        feature_json[f_seg_name] = f_path

    tree_path = {x.split(".")[0]: os.path.join(f"{work_dir}/h9_seg8_clade/data/tree", x) for x in
    ...
    for seq, df in merge_meta_dict.items():
        meta_out_path = os.path.join(meta_out, f"{seq}.csv")
        df.to_csv(meta_out_path, index=False)

    cladeRresultJson = {}
    for row in output_df.values.tolist():
        out_dir = os.path.join(output_dir, "feature")
        os.makedirs(out_dir, exist_ok=True)
        featrue_path = os.path.join(
            out_dir, os.path.basename(feature_json[row[3]])
        )
        ...
        meta_out_path = os.path.join(meta_out, f"{row[3]}.csv")
    ...
    with open(os.path.join(output_dir, "cladeRresultJson.json"), "w") as f:
