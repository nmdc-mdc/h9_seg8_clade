...
def parse_input(input_file, isolate_name,work_dir):
    ...
    output_dir = os.path.join(work_dir, f"/h9_seg8_clade/script/parse_input/parse_output/{isolate_name}")
    ...
    with open(os.path.join(output_dir, "seq_name_dict.json"), "w") as file:
    ...
    for name, seq in seqs.items():
        with open(os.path.join(seqs_path, name + ".fasta"), "w") as file:
            file.write(seq)
