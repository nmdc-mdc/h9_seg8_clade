...
def parse_input(work_dir, input_file, input_isolate_name, python):
    script = os.path.join(work_dir, "/h9_seg8_clade/script/parse_input/parse_input.py")
...
def blastn(work_dir, python, input_directory, output_directory, blastn_path):
    script = os.path.join(work_dir, "/h9_seg8_clade/script/blastn_filter/blastn.py")
...
def qc_and_filter(work_dir, python, parse_fasta, blast_output, input_isolate_name):
    script = os.path.join(work_dir, "/h9_seg8_clade/script/filter/qc_and_filter.py")
    cmd = (
...
def call_snp_and_annotation(work_dir, python, input_isolate_name,mafft_path):
    script = os.path.join(work_dir,"/h9_seg8_clade/script/call_snp_and_annotation/call_snp_and_annotation.py")
    cmd = (
...
def seq_vari_new_vari(work_dir, python, input_isolate_name):
    script = os.path.join(work_dir,"/h9_seg8_clade/script/seq_vari_new/seq_vari_new_vari.py")
    cmd = (
...
def generate_output(work_dir, python, input_isolate_name):
    script = os.path.join(work_dir,"/h9_seg8_clade/script/generate_output.py")
...
def main(work_dir, input_file, python, input_isolate_name, blastn_path,mafft_path):
    ...
    parse_fasta = os.path.join(work_dir, f"/h9_seg8_clade/script/parse_input/parse_output/{input_isolate_name}/seqs")
    blast_output = os.path.join(work_dir, f"/h9_seg8_clade/script/blastn_filter/blast_output/{input_isolate_name}")
...
if __name__ == "__main__":
    ...
    blastn = "F:/software/blast/blast-2.16.0+/bin/blastn.exe"
    # the path of the mafft
    mafft = "F:/software/mafft/mafft-7.526-win64-signed/mafft-win/mafft.bat"
    ...
    parse_input_path = os.path.join(work_dir, "/h9_seg8_clade/script/parse_input/parse_output/")
    if os.path.exists(parse_input_path):
        shutil.rmtree(parse_input_path)
    os.makedirs(parse_input_path)

    blastn_path = os.path.join(work_dir, "/h9_seg8_clade/script/blastn_filter/blast_output")

    if os.path.exists(blastn_path):
        shutil.rmtree(blastn_path)
    os.makedirs(blastn_path)

    filter_result_path = os.path.join(work_dir, "/h9_seg8_clade/script/filter/filter_result")
    if os.path.exists(filter_result_path):
        shutil.rmtree(filter_result_path)
    os.makedirs(filter_result_path)

    call_snp_work_path = os.path.join(work_dir, "/h9_seg8_clade/script/call_snp_and_annotation/work_dir")
    if os.path.exists(call_snp_work_path):
        shutil.rmtree(call_snp_work_path)
    os.makedirs(call_snp_work_path)

    seq_vari_work = os.path.join(work_dir, "/h9_seg8_clade/script/seq_vari_new/work_dir")
    if os.path.exists(seq_vari_work):
        shutil.rmtree(seq_vari_work)
    os.makedirs(seq_vari_work)

    output_path = os.path.join(work_dir, "/h9_seg8_clade/script/output")
    ...
    df_genotype = pd.read_csv(os.path.join(work_dir, "/h9_seg8_clade/data/genotype.csv"),
                              keep_default_na=False,
                              low_memory=False,
                              index_col=False)
    df_genotype = df_genotype.astype(str)
    output_json_path = os.path.join(work_dir, "/h9_seg8_clade/script/output/cladeRresultJson.json")
    output_no_pass_file = os.path.join(work_dir, "/h9_seg8_clade/script/output/no_pass_number.json")
    no_pass_file = {}

    filter_results = os.listdir(filter_result_path)
    for file_name in filter_results:
        file_dir = os.path.join(filter_result_path, file_name)
        no_pass_file_path = os.path.join(file_dir, "qc_filter_not_pass.json")
        pass_file_path = os.path.join(file_dir, "qc_filter_pass.json")
        if file_name == "none":
    ...
    for output_dir in output_lists:
        ...
        output_file = os.path.join(output_dir_path, "cladeRresultJson.json")
