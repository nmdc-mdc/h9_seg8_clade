...
def get_annotation_meta(praperation_path):
    ...
    with open(os.path.join(praperation_path,"non_cds_site.json")) as file:
        non_cds_base_dict = json.load(file)

    with open(os.path.join(praperation_path,"base_amino_site.json")) as file:
...
def multiprocessing_annotation(work_path,
    ...
    paths = [
            [os.path.join(differ_site_path,x),
             os.path.join(annotation_result_path,x.split(".")[0]+".csv")] 
