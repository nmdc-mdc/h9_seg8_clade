...
def multiprocessing_align_seqs(query_ref_file_path, work_path, multiprocessing_number, mafft_path):
    ...
    align_result_path = os.path.join(work_path, "align_result")
    ...
    input_file_paths = [os.path.join(query_ref_file_path, x) for x in query_ref_files]
    result_file_paths = [os.path.join(align_result_path, x) for x in query_ref_files]
...
def multiprocessing_differ_site(work_path, multiprocessing_number):
    """ Align result to differ site """
    align_reuslt_path = os.path.join(work_path, "align_result")
    differ_result_path = os.path.join(work_path, "differ_site")
    if os.path.exists(differ_result_path):
        shutil.rmtree(differ_result_path)
    os.mkdir(differ_result_path)

    align_result_files = [os.path.join(align_reuslt_path, x) for x in os.listdir(align_reuslt_path)]
    differ_result_files = [os.path.join(differ_result_path, x.split(".")[0] + ".csv") for x in
