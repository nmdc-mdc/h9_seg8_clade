...
def split_reference_files(input_file_path,work_path):
    ...
    reference_file_path = os.path.join(input_file_path,"reference_seq.fasta")
    references_directory_path = os.path.join(praperation_directory_path,"references")
    if os.path.exists(references_directory_path):
        shutil.rmtree(references_directory_path)
    os.mkdir(references_directory_path)
    reference_list_path = os.path.join(praperation_directory_path,"reference_list.txt")
    ...
    for ref in refs:
        ...
        with open(os.path.join(references_directory_path,ref_name+".fasta"),mode="a") as file:
            file.write(">"+ref)
...
def gene_site_base(input_file_path):
    ...
    gene_region_file = os.path.join(input_file_path,"gene_regions.txt")
    references_file = os.path.join(input_file_path,"reference_seq.fasta")
...
def base_amino_sites(input_file_path):
    ...
    cds_region_file = os.path.join(input_file_path,"cds_regions.txt")
    references_file = os.path.join(input_file_path,"reference_seq.fasta")
...
def split_query_add_reference(input_file_path,praperation_directory_path,isolate_name,work_dir):
    ...
    input_querys_path = os.path.join(work_dir,f"/h9_seg8_clade/script/call_snp_and_annotation/work_dir/{isolate_name}/input")
    querys_files = os.path.join(input_querys_path,"querys")
    query_seqs_path = os.path.join(praperation_directory_path,"query_seqs")
    references_path = os.path.join(praperation_directory_path,"references")
    ...
    with open(os.path.join(praperation_directory_path,"reference_list.txt")) as file:
    ...
    if len(reference_list) == len(reference_content):
        for i in reference_list:
            if os.path.exists(os.path.join(querys_files,i+"___querys.fasta")):
                # Read the reference sequences.
                with open(os.path.join(references_path,i+".fasta")) as file:
                    reference = file.read().strip()
                # Read the query sequences.
                with open(os.path.join(querys_files,i+"___querys.fasta")) as file:
                ...
                for query in querys:
                    ...
                    if ">" in query_reference:
                        ...
                        with open(os.path.join(query_seqs_path,str(split_query_number)+".fasta"),mode="a") as file:
...
if __name__ == "__main__":
    ...
    base_amino_sites_path = os.path.join(praperation_directory_path,"base_amino_site.json")
    ...
    non_cds_site_json_path = os.path.join(praperation_directory_path,"non_cds_site.json")
